...
import { User as UserIcon, Package, Settings, LogOut, ChevronRight, Eye, EyeOff, Save, X, ChevronDown, Calendar, Mail, Shield } from 'lucide-react';
...
const Profile: React.FC = () => {
    ...
    const [passwordChangeStep, setPasswordChangeStep] = useState<number>(0); // 0: idle, 1: code sent, 2: verified
    ...
    const [selectedAddressCountry, setSelectedAddressCountry] = useState<Country | null>(null);
    ...
    const fetchCities = async (countryCode: string) => {
        ...
        try {
            const response = await api.get<string[]>(`/countries/${countryCode}/cities`);
    ...
    const fetchOrders = async () => {
        ...
        try {
            const response = await api.get<Order[]>('/orders/user/myorders');
    ...
    const formatPhoneNumber = (value: string, format: string | undefined): string => {
    ...
    const handlePhoneCountryChange = (e: ChangeEvent<HTMLSelectElement>) => {
        ...
        setSelectedPhoneCountry(country || null);
        setFormData(prev => ({
            ...prev,
    ...
    const handleAddressCountryChange = (e: ChangeEvent<HTMLSelectElement>) => {
        const countryCode = e.target.value;
        const country = countries.find(c => c.code === countryCode);
        setSelectedAddressCountry(country || null);
        setFormData(prev => ({
            ...prev,
            address: {
                ...prev.address,
    ...
    const getStatusInfo = (order: Order): StatusInfo => {
