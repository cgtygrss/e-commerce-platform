...
router.post('/create-payment', protect, async (req: AuthRequest<{}, {}, CreatePaymentBody>, res: Response): Promise<void> => {
  try {
    const {
      orderItems,
      shippingAddress,
    ...
    // User IP
    const user_ip = (req.headers['x-forwarded-for'] as string) || req.socket.remoteAddress || '127.0.0.1';
...
router.post('/callback', async (req: Request<{}, {}, PayTRCallbackBody>, res: Response): Promise<void> => {
  try {
    const {
      merchant_oid,
      status,
    ...
    if (hash !== calculated_hash) {
      ...
      res.send('PAYTR notification received - Hash mismatch');
      return;
    }
...
router.post('/create-order', protect, async (req: AuthRequest<{}, {}, CreateOrderBody>, res: Response): Promise<void> => {
  try {
    const {
      orderItems,
      shippingAddress,
...
router.get('/status/:merchant_oid', protect, async (req: AuthRequest<{ merchant_oid: string }>, res: Response): Promise<void> => {
  try {
