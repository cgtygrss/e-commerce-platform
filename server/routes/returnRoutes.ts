...
import ReturnRequest, { IReturnItem } from '../models/ReturnRequest';
...
router.get('/my-returns', protect, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.post('/', protect, async (req: AuthRequest<{}, {}, CreateReturnBody>, res: Response): Promise<void> => {
  try {
    ...
    if (!order.isPaid) {
      res.status(400).json({ message: 'Ödenmemiş siparişler için iade talebi oluşturulamaz' });
      return;
    }
    ...
    const daysSinceOrder = Math.floor((Date.now() - new Date(referenceDate).getTime()) / (1000 * 60 * 60 * 24));
...
router.put('/:id/cancel', protect, async (req: AuthRequest<{ id: string }>, res: Response): Promise<void> => {
  try {
    const returnRequest = await ReturnRequest.findOne({
      _id: req.params.id,
      user: req.user!._id,
    ...
    if (!returnRequest) {
      res.status(404).json({ message: 'İptal edilebilir iade talebi bulunamadı' });
      return;
    }
...
router.put('/:id/tracking', protect, async (req: AuthRequest<{ id: string }, {}, TrackingBody>, res: Response): Promise<void> => {
  try {
    ...
    if (!returnRequest) {
      res.status(404).json({ message: 'Onaylanmış iade talebi bulunamadı' });
      return;
    }
...
router.get('/admin/all', protect, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.put('/admin/:id', protect, async (req: AuthRequest<{ id: string }, {}, UpdateReturnBody>, res: Response): Promise<void> => {
  try {
    if (!req.user!.isAdmin) {
      res.status(403).json({ message: 'Yetkiniz yok' });
      return;
